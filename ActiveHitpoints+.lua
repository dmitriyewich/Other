...
local required_offsets = {
    'SAMP_INFO_OFFSET',
    'SAMP_INFO_OFFSET_Pools',
    'SAMP_INFO_OFFSET_Pools_Player',
    'SAMP_SLOCALPLAYERID_OFFSET',
    'MAX_PLAYER_ID_STREAMED_ONLY_OFFSET',
    'SAMP_PREMOTEPLAYER_OFFSET',
    'SAMP_REMOTEPLAYERDATA_OFFSET',
    'SAMP_REMOTEPLAYERDATA_ACTOR',
    'GTA_PED_HANDLE',
...
local function hasRequiredOffsets(version)
    if type(version) ~= 'string' then
        return false
    end

    for i = 1, #required_offsets do
        local offsets = main_offsets[required_offsets[i]]
        if type(offsets) ~= 'table' or offsets[version] == nil then
            return false
        end
    end
...
function main()
    ...
    if currentVersion == 'UNKNOWN' or not hasRequiredOffsets(currentVersion) then
        return thisScript():unload()
    end

    repeat wait(100) until isSAMPInitialized()
    ID_Find = ffi.cast("unsigned short (__thiscall *)(intptr_t, intptr_t)", sampModule + main_offsets.ID_Find[currentVersion])
...
function getColorByHP(hp)
    if hp <= 1 then
        return 0, 0, 0
    elseif hp <= 10 then
        return lerpColor(hp / 10, 0, 0, 0, 255, 0, 0)
    elseif hp <= 50 then
        return lerpColor((hp - 10) / 40, 255, 0, 0, 255, 255, 0)
    elseif hp <= 100 then
        return lerpColor((hp - 50) / 50, 255, 255, 0, 25, 255, 25)
    else
        return 25, 255, 25
    end
end
...
function set_triangle_color(r, g, b)
    r = math.floor((tonumber(r) or 0) + 0.5)
    g = math.floor((tonumber(g) or 0) + 0.5)
    b = math.floor((tonumber(b) or 0) + 0.5)
